...
pub const Model = struct {
    ...
    pub fn loadFromFile(path: [*:0]const u8) !Model {
        return .{
            .ptr = c.llama_load_model_from_file(path, c.llama_context_default_params()) orelse return error.InvalidModel,
        };
    }
...
pub const Sampler = struct {
    ...
    pub fn sample(self: *Sampler, ctx: *Context) !Token {
        ...
        try self.candidates.ensureTotalCapacity(@intCast(c.llama_n_vocab(ctx.ptr)));
        self.candidates.expandToCapacity();
