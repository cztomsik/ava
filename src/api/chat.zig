...
pub fn @"PUT /chat/:id"(id: u32, data: db.Chat) !void {
...
pub fn @"PUT /chat/:id/messages/:message_id"(id: u32, message_id: u32, data: db.ChatMessage) !void {
    try db.exec("UPDATE ChatMessage SET role = ?, content = ? WHERE id = ? AND chat_id = ?", .{ data.role, data.content, message_id, id });
}

pub fn @"DELETE /chat/:id/messages/:message_id"(id: u32, message_id: u32) !void {
    try db.exec("DELETE FROM ChatMessage WHERE id = ? AND chat_id = ?", .{ message_id, id });
}

pub fn @"DELETE /chat/:id"(id: u32) !void {
